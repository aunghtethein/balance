...
<body>
	...
	<main class="employee">
		<header class="shadow">
			...
			<nav>
				 <a class="<%= getActiveClass(request, "home") %>" href="<%=getPath("/employee/home")%>">
				 	<img src="<%= getSvg("meter") %>" class="icon icon-left"  />
				 Dashboard
				 </a>
				...
				<% if(loginInfo.isManager()) { %> 
				 ...
				 <img src="<%= getSvg("stats-dots") %>" class="icon icon-left"  />
				 Balance Report</a>
				 
				 <a class="<%= getActiveClass(request, "employees") %>" href="<%=getPath("/manager/employee/search")%>">
				 <img src="<%= getSvg("users") %>" class="icon icon-left"  />
				 Employees</a> 
				<%}%>
				
				 <a href="<%=getPath("/logout")%>">
				 <img src="<%= getSvg("exit") %>" class="icon icon-left"  />
				 Logout</a>
			</nav>
		</header>
		...
		<aside class="box shadow">
			...
			<!-- User login profile -->
			<img class="profile-img" src="<%= getPath("/assets/images/" + loginInfo.getProfileImage()) %>" />
			...
			<div class="user-actions">
				<a class="btn" onclick="openUploadFile(); return false;">Upload Photo</a>
				<a class="btn" href="<%= getPath("/employee/edit-profile")%>">Edit Profile</a>
				<a class="btn" href="<%= getPath("/employee/change-pass")%>">Change Password</a>
			</div>
			
			<form id="profileImageForm" action="<%= getPath("/employee/profile-image") %>" method="post" enctype="multipart/form-data">
